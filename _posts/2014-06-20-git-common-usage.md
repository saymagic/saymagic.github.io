---
layout: post
keywords: git，用法，.DS_Store,.gitignore
...
(将本地的master分支推送到远程master分支上，是`git push origin master:master` 的简写形式)
...
```
	...
	对于这种常用却又比较长的命令，我习惯将其组装成函数加入bashrc中
...
* git 默认对于文件名大小写是不敏感的,要想修正这一点，使用如下配置：

```
git config  core.ignorecase false
```

* 有时候commit之后突然意识到commit的信息中有单词拼错之类，想做修改：

```
git commit --amend
```

* Vim在安装Vundle插件后使用Vim来commit一些信息可能会报错如下
...
有些时候我们在clone一个项目编译后，往往ide会生成许多文件，我们需要将其加入.gitignore,但我们按照规则添加进去之后往往并不会生效，原因是.gitignore只能忽略那些原来没有被track的文件该文件只能作用于 Untracked Files，也就是那些从来没有被 Git 记录过的文件（自添加以后，从未 add 及 commit 过的文件)，如果某些文件已经被纳入了版本管理中，则修改.gitignore是无效的。那么解决方法就是先把本地
缓存删除（改变成未track状态）：
